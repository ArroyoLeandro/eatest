...
function PaymentData({ formData, setFormData, initForm, position }) {
    ...
    const [acceptedCardList, setAcceptedCardList] = useState([])

    const [country, setCountry] = useState(Cookie.get('location')??'lat')
    ...
    useEffect(() => {
        if(formData.mesVencimiento != '') setVencimiento(formData.mesVencimiento+'/'+formData.anioVencimiento)
        setDatoPersona(formData)


    // si es argentina dejar default, si es otro pais consultar metodos de pago
    ...
    GTM.begin_checkout(dataEcommerce)
    ...
    const getMetodosPago = async ()=> {
        ...
        try {
            let user = Cookie.get('location').toUpperCase()
              let response = await axios.get(process.env.NEXT_PUBLIC_URL_METODOS_PAGO+'?codigoUsuario='+user);
              let res = response.data[0]
              let cards = []
              res.map(item=>{
                cards.push({name:item.name,id:item.id})
              })
              setAcceptedCardList(cards)
          }
          catch (e) {
        ...
        <>
            <div className='flex flex-col'>
                ...
                <div className='flex flex-col gap-2 py-6 px-6 rounded-lg shadow-3xl overflow-hidden'>
                    <p className="flex-none col-span-full mb-2">{t("formContratar.formaPagoSubtitulo")}</p>
                    <div className="flex flex-col sm:flex-row gap-10">
                    {
                        metodoPago?.toLowerCase() == 'decidir' ?
                        <>
                        ...
                        <Cards 
                            ...
                            acceptedCards={['visa','mastercard','amex']} //ver como hacer para que la peticion de tarjetas me active o desactive las tarjetas
                        ...
                        {metodoPago?.toLowerCase() != 'decidir' ?
                        <>
                        <label className='mt-4 flex-1 relative group' htmlFor="tipoTarjeta">
                            <select name="tipo" className='appearance-none w-full py-3 px-3 border border-principal rounded bg-white focus:outline-none focus:border-gris-medio' 
                                onChange={(e) => {
                                    setDato('tipoTarjeta', e.target.value);
                                }}>
                                <option value="">Tipo de Tarjeta</option>
                            {
                                acceptedCardList.map((item)=>{
                            ...
                            <span className={`absolute pointer-events-none left-4 px-1 text-gris-medio transition-all ease-out origin-left whitespace-nowrap group-focus-within:-translate-y-2 group-focus-within:scale-75 group-focus-within:bg-white group-focus-within:py-0 top-0 ${ formData?.tipoTarjeta != '' ? 'scale-75 -translate-y-2 bg-white' : 'opacity-0'} `}>{t("formContratar.tipoTarjeta")}</span>
                        </label>
                        <div id="card-field" className="mt-4 flex-1 relative group border border-principal py-3 px-1 w-full text-principal text-xl font-bold"></div>
                        </>
                        : null}
